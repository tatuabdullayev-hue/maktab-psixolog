...
const QUESTIONS: TestQuestion[] = [
  {
    id: 'q1',
    domain: 'aggression',
    text: "Sinfdoshing sendan kulib, hamma oldida masxara qildi. Nima qilasan?",
    options: [
  ...
  },
  {
    id: 'q2',
    domain: 'bullying',
    text: 'Tanaffusda bir guruh o\'quvchi yangi kelgan o\'qishchini doim masxara qiladi. Sen buni ko\'rib turasan.',
    options: [
  ...
  },
  {
    id: 'q15',
    domain: 'conduct',
    text: 'Agar yerda kimnikidir pulini topib olsang, nima qilasan?',
    options: [
...
async function seed() {
  ...
  const existingTest = await testRepo.findOne({
    where: { title: "Sarguzasht: O'zingni tanish" },
  });
  if (!existingTest) {
    await testRepo.save(
      testRepo.create({
        title: "Sarguzasht: O'zingni tanish",
        description:
    ...
    console.log("Test yaratildi: Sarguzasht: O'zingni tanish");
  }
