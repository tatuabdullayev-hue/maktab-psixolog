...
function cell(text: string, isHeader = false, width = 2000): TableCell {
...
function noBorderCell(children: Paragraph[], width = 4500): TableCell {
  return new TableCell({
    width: { size: width, type: WidthType.DXA },
    borders: NO_BORDER,
    children,
...
export class WordReportService {
  async generate(data: WordReportData): Promise<Buffer> {
    ...
    const recentNotes = data.notes.filter(
      (n) => new Date(n.createdAt) >= sixMonthsAgo,
    );
    ...
    const attentionStudents = data.students.filter(
      (s) => s.level === 'attention',
    );
      ...
      sections: [
        {
          ...
          children: [
            // ════ SARLAVHA ════
            p(
              [run("O'ZBEKISTON RESPUBLIKASI", { bold: true })],
              { before: 0, after: 60 },
              AlignmentType.CENTER,
            ),
            p(
              [run(`NAMANGAN VILOYATI ${data.district.toUpperCase()}`, { bold: true })],
              { before: 0, after: 60 },
              AlignmentType.CENTER,
            ),
            p(
              [run(`${data.school.toUpperCase()} MAKTAB PSIXOLOGI`, { bold: true })],
              { before: 0, after: 200 },
              AlignmentType.CENTER,
            ),
            p(
              [run('HISOBOT', { bold: true, size: 36 })],
              { before: 0, after: 60 },
              AlignmentType.CENTER,
            ),
            p(
              [
                run(
                  "Jinoyatchilik va deviant xulqni barvaqt oldini olish bo'yicha",
                  { bold: true, size: 24 },
                ),
              ],
              { before: 0, after: 60 },
            ...
            p(
              [run(`Hisobot davri: ${period}`, { italics: true })],
              { before: 0, after: 400 },
              AlignmentType.CENTER,
            ),
            ...
            new Table({
              ...
              rows: [
                new TableRow({
                  children: [
                    cell("Ko'rsatkich", true, 4500),
                    cell('Qiymat', true, 4500),
                  ],
                }),
                new TableRow({
                  children: [cell('Maktab'), cell(data.school)],
                }),
                new TableRow({
                  children: [cell('Tuman'), cell(data.district)],
                }),
                new TableRow({
                  children: [cell('Hisobot davri'), cell(period)],
                }),
                new TableRow({
                  children: [
                    cell("Jami test topshirgan o'quvchilar"),
                    cell(`${data.total} nafar`),
                  ],
                }),
                new TableRow({
                  children: [
                    cell('Hisobot tuzilgan sana'),
                    cell(generatedAt),
                  ],
                }),
              ],
            ...
            p([
              run(
                "AI tizimi (Claude Sonnet) tomonidan o'tkazilgan psixologik tahlil natijalari:",
                { bold: true },
              ),
            ]),
            new Table({
              ...
              rows: [
                new TableRow({
                  children: [
                    cell('Risk darajasi', true, 3000),
                    cell("O'quvchilar soni", true, 3000),
                    cell('Ulushi (%)', true, 3000),
                  ],
                }),
                new TableRow({
                  children: [
                    cell('Yuqori xavf (DANGER)'),
                    cell(`${data.danger} nafar`),
                    cell(pct(data.danger, data.total)),
                  ],
                }),
                new TableRow({
                  children: [
                    cell("O'rta xavf (ATTENTION)"),
                    cell(`${data.attention} nafar`),
                    cell(pct(data.attention, data.total)),
                  ],
                }),
                new TableRow({
                  children: [
                    cell('Past xavf (NORMAL)'),
                    cell(`${data.normal} nafar`),
                    cell(pct(data.normal, data.total)),
                  ],
                }),
                new TableRow({
                  children: [
            ...
            ...(dangerStudents.length === 0
              ...
              : [
                  p([
                    run(
                      `Quyida yuqori xavf (DANGER) darajasi aniqlangan ${dangerStudents.length} nafar o'quvchi ro'yxati va AI tizimi xulosasi keltirilgan:`,
                    ),
                  ]),
                  new Table({
                    width: { size: 9000, type: WidthType.DXA },
                    rows: [
                      new TableRow({
                        children: [
                          cell('Nr', true, 600),
                          cell('F.I.O.', true, 2200),
                          cell('Sinf', true, 700),
                          cell('AI xulosasi', true, 5500),
                        ],
                      }),
                      ...dangerStudents.map(
                        (s, i) =>
            ...
            p([
              run(
                `Jami kiritilgan ish yozuvlari: ${recentNotes.length} ta`,
                { bold: true },
              ),
            ]),
            ...Object.entries(typeStats).map(([type, count]) =>
              p(
                [run(`• ${TYPE_LABELS[type] ?? type}: ${count} ta`)],
                { before: 40, after: 40 },
              ),
            ),
